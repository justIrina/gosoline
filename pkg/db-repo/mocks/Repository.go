...
// WithTransaction provides a mock function with given fields: transaction
func (_m *Repository) WithTransaction(transaction db_repo.Transaction) (*db_repo.repository, error) {
	ret := _m.Called(transaction)

	var r0 *db_repo.repository
	if rf, ok := ret.Get(0).(func(db_repo.Transaction) *db_repo.repository); ok {
		r0 = rf(transaction)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*db_repo.repository)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(db_repo.Transaction) error); ok {
		r1 = rf(transaction)
	} else {
		r1 = ret.Error(1)
	}
